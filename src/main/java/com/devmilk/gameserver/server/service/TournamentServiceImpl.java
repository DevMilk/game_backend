...
import com.devmilk.gameserver.server.repository.ChatGroupRepository;
...
public class TournamentServiceImpl implements TournamentService {
    ...
    private static final Boolean[] groupCreationHashMap;

    static {
        groupCreationHashMap = new Boolean[Math.floorDiv(GAME_SETTINGS.MAX_LEVEL, GAME_SETTINGS.GROUP_LEVEL_RANGE) + 1];
        Arrays.fill(groupCreationHashMap, Boolean.FALSE);
    }
    ...
    private int calculateReward(int userRank) {
        ...
        for (GAME_SETTINGS.RewardEnum place : GAME_SETTINGS.RewardEnum.values())
    ...
    private Boolean getIsUserHaveARewardToClaim(User user) {
        ...
        Optional<LeaderboardRecord> lastRecordOfUser = leaderboardRepository
                .getLastLeaderboardRecordOfUser(user.getUserId());

        //If no record found, then user have never entered a tournament so cant have a reward to claim
        if (lastRecordOfUser == null || !lastRecordOfUser.isPresent())
            return Boolean.FALSE;

        //Get group from leaderboard record of user
        TournamentGroup lastEnteredTournamentGroupOfUser = lastRecordOfUser.get().getGroupId();
    ...
    private TournamentGroup createOrGetTournamentGroupAndInsertNewUser(Long tournamentDay, int userLevelRange, User user) {
        // Create new tournament group
        TournamentGroup group = tournamentGroupRepository.getLastCreatedGroupWithGivenLevelRange(userLevelRange);

        //If group full or not exists or not today's daily, create new one
        if (group == null || group.getLeaderboard().size() >= GAME_SETTINGS.GROUP_SIZE_LIMIT
    ...
    public List<LeaderboardRecord> register(Long userId) {
        ...
        if (userCoins < GAME_SETTINGS.TOURNAMENT_ENTRANCE_FEE)
            throw new ConditionsDoesntMetException("User must pay " + GAME_SETTINGS.TOURNAMENT_ENTRANCE_FEE + " coins to enter a tournament");

        if (userLevel < GAME_SETTINGS.TOURNAMENT_ENTRANCE_LEVEL_REQUIREMENT)
            throw new ConditionsDoesntMetException("User's level must be more or equal than " +
                    GAME_SETTINGS.TOURNAMENT_ENTRANCE_LEVEL_REQUIREMENT + " to enter a tournament");
        ...
        int userLevelRange = Math.floorDiv(userLevel, GAME_SETTINGS.GROUP_LEVEL_RANGE);
         ...
         */
        synchronized (groupCreationHashMap[userLevelRange]) {
            group = createOrGetTournamentGroupAndInsertNewUser(currentDay, userLevelRange, user);
        }


        //Reset reward claim of user and withdraw enterance fee
        user.setIsClaimedLastReward(Boolean.FALSE);
        userProgress.setCoins(userCoins - GAME_SETTINGS.TOURNAMENT_ENTRANCE_FEE);
